...
class AddProfile extends _$AddProfile with AppLogger {
  ...
  Future<void> add(String rawInput) async {
    ...
    state = await AsyncValue.guard(
        ...
        // Check if rawInput can be parsed as a link
        if (LinkParser.parse(rawInput) case (final link)?) {
          loggy.debug("adding profile, url: [${link.url}]");

          // Validate the URL contains the domain "ipman.cc"
          if (!link.url.contains("ipman")) {
            loggy.debug("URL does not contain 'ipman'");
          ...
          // Check if rawInput matches a specific protocol
          ...
          // If neither parsing succeeds, throw an error
        } else {
